...
class DataFeedSentinel(BaseSentinel):
    ...
    def check(self) -> SentinelResult:
        try:
            from falcon_core.backtesting.data_feed import DataFeed, NoTradingSessionError
        ...
        probe_date = probe_session()
        try:
            data = feed.get_historical_data(
                PROBE_SYMBOL, probe_date, probe_date,
                interval="1d", source="flatfiles",
            )
        except NoTradingSessionError as e:
            # The market was shut, not the feed. Reporting FAIL here is what
            # made the sentinel page read 6/8 for a week after Labor Day
            # (falcon-core#20).
            return SentinelResult(
                name=self.name,
                status=SentinelStatus.SKIP,
                reason=f"{probe_date} is not a trading session: {e}",
