...
def main():
    ...
    if args.json_output:
        ...
        skipped = sum(1 for r in results if r.status == SentinelStatus.SKIP)
        # Skipped checks are excluded from the denominator. On a market
        # holiday the flat-file sentinels cannot run at all, and counting
        # them against the total reads as though something is broken.
        applicable = total - skipped
        print(f"  {passed}/{applicable} passed", end="")
        if warned:
